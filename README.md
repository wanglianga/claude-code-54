...
- 端到端自测（可选）：栈启动后执行 `node scripts/smoke.mjs`（默认打 `http://host.docker.internal:3054`，可用 `BASE=http://host.docker.internal:<端口>` 覆盖），覆盖登录、推荐、配件联动拦截、签到取证、报价确认、维修完成、支付评价归档、质保返修、异常协同、统计准入、档案追溯共 46 项断言。
