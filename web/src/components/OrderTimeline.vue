...
function detailText(d: any): string {
  ...
  if (d.location) parts.push(`坐标：${d.location}`)
  if (d.anchor_condition) parts.push(`固定条件：${d.anchor_condition}`)
  if (d.need_two_person) parts.push('需双人作业')
  if (d.danger_desc) parts.push(`危险情况：${d.danger_desc}`)
